...
export const movecarTags = pgTable(
  ...
  (table) => {
    return {
      userIdx: index('idx_movecar_tags_user_id').on(table.userId),
      shortCodeIdx: index('idx_movecar_tags_short_code').on(table.shortCode),
    }
  }
)
...
export const movecarNotifications = pgTable(
  'movecar_notifications',
  {
    ...
    tagId: uuid('tag_id')
      .notNull()
      .references(() => movecarTags.id, { onDelete: 'cascade' }),
  ...
  (table) => {
    return {
      tagIdx: index('idx_movecar_notif_tag_id').on(table.tagId),
      ipCreatedIdx: index('idx_movecar_notif_ip_created').on(
        table.requesterIp,
        table.createdAt
      ),
    }
...
export const movecarPushTokens = pgTable(
  ...
  (table) => {
    return {
      userChannelIdx: index('idx_movecar_push_tokens_user_channel').on(
        table.userId,
        table.channel
      ),
    }
